...
class Catapulte:
	...
	def click(self, event):
		...
		print("Click x ->", souris_x,"\nClick y ->", souris_y - background.posy - 200)

		if self.posx <= souris_x <= self.posx + self.largeur_img and self.posy <= souris_y <= self.posy + self.hauteur_img: #si je clique sur la catapulte alors on met self.drag à True
			self.drag = True
	...
	def trajectoire(self):
		...
		self.point = canvas.create_oval(self.posx_proj - 15, self.posy_proj - 15, self.posx_proj, self.posy_proj, fill = 'white', outline = self.color_point)
	...
	def check_collision(self):
		...
		if catapulte2.posx < catapulte1.posx_proj and catapulte2.posy < catapulte1.posy_proj:
			print("collide")
			#ici faire le systeme de vie/collide.. etc
	...
	def return_camera(self):
		...
		if compteur == 0:
			if tour == 0:
				canvas.itemconfigure(text_tour, text = "TOUR DU JOUEUR 2", font = ('angrybirds', 35))
				diff_longueur = (catapulte1.posx - background.posx) - catapulte2.posx
				print("longueur diff", diff_longueur)
				tour = 1

			elif tour == 1:
				canvas.itemconfigure(text_tour, text = "TOUR DU JOUEUR 1", font = ('angrybirds', 35))
