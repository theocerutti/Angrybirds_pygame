	...
	def drag_obj(self, event):
		...
		if self.drag == True:
			if time.time() - self.last_run > self.period_anim: #cette ligne est pour limiter la vitesse d'animation
				#l'animation
				if self.souris_y < souris_y:
					print("souris vers le bas ")
					self.cata_indice -= 1

				elif self.souris_y > souris_y:
					self.cata_indice += 1
					print("souris vers le haut")

				print(self.cata_indice)

				if self.cata_indice >= len(self.cata_list): #si cata_indice est au dernier indice de la liste alors on le remet à 0
					self.cata_indice = 11
				elif self.cata_indice <= -len(self.cata_list):
					self.cata_indice = -11
