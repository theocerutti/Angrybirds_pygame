...
red_bird_mirror_img = PhotoImage(file = "lib/image/red_bird_mirror_mini.gif")
...
class Catapulte:
	...
	def __init__(self, posx, posy, image, x0ela, y0ela, x1ela, y1ela, x2ela, y2ela, x3ela, y3ela, posx_projectile, posy_projectile, image_projectile, image_part, posx_part, posy_part):
	...
	def shoot(self):
		...
		if self.posy_proj <= 520:
			...
			if self.posy_proj >= 519:
				time.sleep(1.5) #on attend 2 secondes..
				self.return_camera()
	...
	def move_left(self, event):
		"""Fonction permettant de bouger 2 fois vers la gauche PENDANT son tour."""

		print("move left")
		self = catapulte1

		if self.posx < 250 - background.posx: #quand je sort de la zone alors le move = 0
			move = 0
		else: #sinon je le met a 50
			move = -10
		#on ajoute 100 à toutes les coordonnées en x de la catapulte/proj
		self.posx += move
		self.posx_part += move
		self.x0ela += move
		self.x1ela += move
	...
	def move_right(self, event):
		"""Fonction permettant de bouger 2 fois vers la droite PENDANT son tour."""

		print("move right")
		self = catapulte1

		if self.posx > 590 - self.largeur_img + background.posx: #quand je sort de la zone alors le move = 0
			move = 0
		else: #sinon je le met a 50
			move = 10
		#on ajoute 100 à toutes les coordonnées en x de la catapulte/proj
	...
	def return_camera(self):
		...
		global tour, compteur, shift_return, diff_longueur, text_tour

		if compteur == 0:
			if tour == 0:
				canvas.itemconfigure(text_tour, text = "TOUR DU JOUEUR 2")
				diff_longueur = (catapulte1.posx - background.posx) - catapulte2.posx
				print("longueur diff", diff_longueur)
				tour = 1

			elif tour == 1:
				canvas.itemconfigure(text_tour, text = "TOUR DU JOUEUR 1")
				diff_longueur = -background.posx
				print("longueur diff", diff_longueur)
				tour = 0
		...
		catapulte1.posx_part += shift_return
		...
		catapulte2.posx_part += shift_return
...
background = World(0, -200, background_img)

catapulte1 = Catapulte(368, 420, catapulte_left_img, 375, 439, 377, 441,   402, 439, 404, 441,     371, 420, red_bird_img, part_cata_left_img, 368, 420)
catapulte2 = Catapulte(1696, 420, catapulte_right_img, 1698, 439, 1700, 441,   1724, 439, 1726, 441,    1699, 420, red_bird_mirror_img, part_cata_right_img, 1696, 420)

#text des tours
text_tour = canvas.create_text(420,100,fill="black", font = ('segoe UI black', 35), text="TOUR DU JOUEUR 1")
...
fenetre.bind("<Left>", catapulte1.move_left) #mouvement gauche, flèche gauche
fenetre.bind("<Right>", catapulte1.move_right) #mouvement droite, flèche droite
...
fenetre.bind("q", catapulte2.move_left) #mouvement gauche, touche Q
fenetre.bind("d", catapulte2.move_right) #mouvement droite, touche D
