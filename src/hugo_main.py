...
arrow_right_img = PhotoImage(file = "lib/image/arrow_right.gif")
arrow_left_img  = PhotoImage(file = "lib/image/arrow_left.gif")
#----------------------------------------------------------------------------------------------------
# VARIABLES
...
class World():
	...
	def __init__(self, posx, posy, image, image_fleche_right, image_fleche_left):
		...
		#flèche
		self.image_fleche_right = image_fleche_right
		self.image_fleche_left = image_fleche_left
		self.image_default_arrow = self.image_fleche_right
		self.posx_arrow = 740
		self.posy_arrow = 340
		self.image_fleche = canvas.create_image(self.posx_arrow, self.posy_arrow, image = self.image_default_arrow, anchor = "nw")
	...
	def arrow_direction(self):
		"""Fonction permettant de pointer le joueur adverse grâce à une flèche"""
		...
		if world.posx_arrow - world.posx - 600 > self.posx:
			world.image_default_arrow = world.image_fleche_left
			canvas.delete(world.image_fleche)
			world.image_fleche = canvas.create_image(world.posx_arrow, world.posy_arrow, image = world.image_default_arrow, anchor = "nw")

		elif world.posx_arrow - world.posx + 600 < self.posx:
			world.image_default_arrow = world.image_fleche_right
			canvas.delete(world.image_fleche)
			world.image_fleche = canvas.create_image(world.posx_arrow, world.posy_arrow, image = world.image_default_arrow, anchor = "nw")
...
world = World(0, -200, background_img, arrow_right_img, arrow_left_img)
